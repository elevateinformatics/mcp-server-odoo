...
class TestServerFoundation:
    ...
    def test_server_version(self):
        """Test server version is set and at least three components long.

        We accept dev suffixes like ``0.0.0-dev`` because the package may be
        running from source without an installed dist (importlib.metadata fallback).
        """
        assert SERVER_VERSION
        parts = SERVER_VERSION.split(".")
        assert len(parts) >= 3, f"Expected x.y.z(.…) format, got {SERVER_VERSION}"
