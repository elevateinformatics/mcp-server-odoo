...
| `ODOO_LOCALE` | No | Default session locale for Odoo responses (mutable at runtime via `set_locale`) | `es_ES`, `es_AR`, `en_US`, `fr_FR` |
...
- When `ODOO_LOCALE` is set, Odoo returns translated field labels, selection values, and messages in that language. The session locale is mutable at runtime via the `set_locale` tool, and per-call `lang` overrides are supported on every CRUD tool.
...
| `ODOO_CACHE_FIELD_TTL` | `3600` | TTL (seconds) for cached field metadata |
| `ODOO_CACHE_PERMISSION_TTL` | `300` | TTL (seconds) for cached MCP permissions |
